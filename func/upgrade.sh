...
#===========================================================================#
#                                                                           #
# LinkData Panel - Upgrade Function Library                           #
#                                                                           #
#===========================================================================#
...
upgrade_health_check() {
	...
	if [ -z "$VERSION" ]; then
		...
		echo
		echo "[ ! ] Unable to detect installed version of LinkData Panel."
...
upgrade_welcome_message() {
	...
	echo "                                                                              "
	echo "                    LinkData Panel Software Update                      "
...
upgrade_welcome_message_log() {
	echo "============================================================================="
	echo "LinkData Panel Software Update Log"
	echo "============================================================================="
	echo
...
upgrade_complete_message() {
	...
	echo
	echo "We hope that you enjoy using this version of LinkData Panel,           "
	echo "have a wonderful day!                                                        "
	echo
	echo "Sincerely,                                                                   "
	echo "The LinkData Panel development team                                    "
	echo
	echo "Web:      https://www.linkdata.com/                                          "
	echo "Docs:     https://docs.hestiacp.com/										   "
	echo "Forum:    https://forum.hestiacp.com/                                        "
	echo "GitHub:   https://github.com/hestiacp/hestiacp/                              "
	echo
	echo "Help support the LinkData Panel project by donating via PayPal:        "
	echo "https://www.linkdata.com/donate                                              "
	echo
...
upgrade_get_version() {
	# Retrieve new version number for LinkData Panel from .deb package
...
upgrade_send_notification_to_panel() {
	...
	if [[ "$new_version" =~ "alpha" ]]; then
		...
		$BIN/v-add-user-notification "$ROOT_USER" 'Development snapshot installed' '<p><span class="u-text-bold">Version:</span> '$new_version'<br><span class="u-text-bold">Code Branch:</span> '$RELEASE_BRANCH'</p><p>Please report any bugs by <a href="https://github.com/hestiacp/hestiacp/issues" target="_blank">opening an issue on GitHub</a>, and feel free to share your feedback on our <a href="https://forum.hestiacp.com" target="_blank">discussion forum</a>.</p><p><i class="fas fa-heart icon-red"></i> The LinkData Panel development team</p>'
	elif [[ "$new_version" =~ "beta" ]]; then
		# Send feedback notification for beta releases
		$BIN/v-add-user-notification "$ROOT_USER" 'Thank you for testing LinkData Panel '$new_version'.' '<p>Please share your feedback with our development team through our <a href="https://forum.hestiacp.com" target="_blank">discussion forum</a>.</p><p>Found a bug? <a href="https://github.com/hestiacp/hestiacp/issues" target="_blank">Open an issue on GitHub</a>!</p><p><i class="fas fa-heart icon-red"></i> The LinkData Panel development team</p>'
	else
		# Send normal upgrade complete notification for stable releases
		$BIN/v-add-user-notification "$ROOT_USER" 'Upgrade complete' '<p>LinkData Panel has been updated to <span class="u-text-bold">v'$new_version'</span>.</p><p><a href="https://github.com/hestiacp/hestiacp/blob/release/CHANGELOG.md" target="_blank">View release notes</a></p><p>Please report any bugs by <a href="https://github.com/hestiacp/hestiacp/issues" target="_blank">opening an issue on GitHub</a>.</p><p class="u-text-bold">Have a wonderful day!</p><p><i class="fas fa-heart icon-red"></i> The LinkData Panel development team</p>'
	fi
}
...
upgrade_send_notification_to_email() {
	...
	if [ "$UPGRADE_SEND_EMAIL" = "true" ]; then
		...
		echo "$HOSTNAME has been upgraded from LinkData Panel v$VERSION to v${new_version}." >> $message_tmp_file
		...
		echo "" >> $message_tmp_file
		echo "Help support the LinkData Panel project by donating via PayPal: https://www.linkdata.com/donate" >> $message_tmp_file
		echo "===================================================" >> $message_tmp_file
		echo "Have a wonderful day," >> $message_tmp_file
		echo "The LinkData Panel development team" >> $message_tmp_file
...
upgrade_init_backup() {
	...
	# LinkData Panel configuration files
...
upgrade_start_routine() {
	...
	if [ $(check_version $latest_version) -gt $(check_version $VERSION) ]; then
		...
		echo ""
		echo "[ ! ] The latest version of LinkData Panel is already installed."
...
upgrade_restart_services() {
	...
	# Always restart the LinkData Panel service
	if [ "$DEBUG_MODE" = "true" ]; then
		echo "      - hestia"
	fi
